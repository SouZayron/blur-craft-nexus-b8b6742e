  ...
  return (
    <>
      ...
      <style>{`
        ...
        .mc-root{--text:#f5ecff;color:var(--text);background:radial-gradient(ellipse at top,#3a1857 0%,#1a0d2e 50%,#0f0820 100%);font-family:'Barlow',sans-serif;min-height:100vh;padding:1rem}
        .mc-wrap{max-width:1400px;margin:0 auto;display:grid;grid-template-columns:1fr 1.2fr 1fr;gap:1rem;align-items:stretch}
        @media (max-width:1100px){.mc-wrap{grid-template-columns:1fr}}
        .mc-header{grid-column:1/-1;text-align:center;margin-bottom:.4rem}
        .mc-logo{font-family:'Bebas Neue',sans-serif;font-size:2.4rem;letter-spacing:5px;background:linear-gradient(135deg,#e9c879,#c9a24a,#ffd1ec);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
        .mc-sub{font-family:'Barlow Condensed',sans-serif;font-size:.75rem;letter-spacing:5px;text-transform:uppercase;color:#bca8d9;margin-top:2px}
        .mc-col{display:flex;flex-direction:column;gap:.7rem;min-width:0}
        .mc-panel{background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);border-radius:14px;padding:.85rem 1rem;backdrop-filter:blur(12px)}
        .mc-ptitle{font-family:'Barlow Condensed',sans-serif;font-size:.75rem;letter-spacing:3px;text-transform:uppercase;color:#bca8d9;margin-bottom:.5rem}
        .mc-input{width:100%;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);border-radius:8px;padding:.5rem .7rem;color:#f5ecff;font-size:.9rem;outline:none;margin-bottom:.5rem}
        .mc-input:focus{border-color:#c47ad9}
        .mc-btn{width:100%;font-family:'Barlow Condensed',sans-serif;font-size:.9rem;letter-spacing:2px;text-transform:uppercase;background:linear-gradient(135deg,#8b3fbf,#c47ad9);color:#fff;border:none;border-radius:8px;padding:.55rem;cursor:pointer;font-weight:700}
        .mc-btn:hover{filter:brightness(1.1)}
        .mc-btn.ghost{background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.2)}
        .mc-rules{font-family:'Barlow Condensed',sans-serif;font-size:.78rem;color:#d9c8ef;line-height:1.55}
        .mc-rules li{margin-bottom:.15rem}
        .mc-pt-row{display:flex;justify-content:space-between;align-items:center;padding:.3rem 0;border-bottom:1px solid rgba(255,255,255,0.06);font-size:.8rem}
        .mc-pt-row img{width:22px;height:22px;vertical-align:middle;margin-right:.4rem}
        ...
        .mc-streak div{font-family:'Barlow Condensed',sans-serif}
        .mc-streak b{display:block;font-family:'Bebas Neue',sans-serif;font-size:1.4rem;color:#e9c879}
        .mc-stats{display:grid;grid-template-columns:1fr 1fr 1fr;gap:.5rem;margin-bottom:.75rem}
        .mc-stat{background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:.5rem;text-align:center}
        .mc-stat-lbl{font-family:'Barlow Condensed',sans-serif;font-size:.6rem;letter-spacing:1.5px;text-transform:uppercase;color:#bca8d9}
        .mc-stat-val{font-family:'Bebas Neue',sans-serif;font-size:1.4rem;letter-spacing:1px;color:#e9c879}
        .mc-reels-frame{background:#060709;border:1px solid #262c38;border-radius:12px;padding:14px;position:relative}
        ...
        .mc-message{margin-top:16px;text-align:center;min-height:22px;font-size:.9rem;color:#bca8d9}
        .mc-message.win{color:#e9c879;font-weight:700}
        .mc-message.empty{color:#c88b8b}
        .mc-spin-btn{display:block;margin:18px auto 4px;width:70px;height:70px;border-radius:50%;background:radial-gradient(circle at 30% 30%,#e9c879,#8b6a1f);border:2px solid #e9c879;cursor:pointer;font-family:'Bebas Neue',sans-serif;font-size:1.1rem;color:#1a0d2e;letter-spacing:2px;box-shadow:0 6px 20px rgba(0,0,0,.6)}
        ...
        .mc-list{max-height:420px;overflow-y:auto;display:flex;flex-direction:column;gap:.3rem}
        .mc-list::-webkit-scrollbar{width:6px}
        .mc-list::-webkit-scrollbar-thumb{background:rgba(196,122,217,0.4);border-radius:3px}
        .mc-rank-row{display:grid;grid-template-columns:24px 1fr auto;gap:.5rem;align-items:center;padding:.35rem .55rem;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:8px;font-size:.85rem}
        ...
        .mc-log-row{display:grid;grid-template-columns:1fr auto auto auto;gap:.4rem;align-items:center;font-family:'Barlow Condensed',sans-serif;font-size:.75rem;padding:.28rem .5rem;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:6px}
        .mc-log-row.trinca{background:rgba(233,200,121,.1);border-color:rgba(233,200,121,.3)}
        .mc-log-syms{display:flex;gap:2px}
        .mc-log-syms img{width:16px;height:16px}
