  ...
  return (
    <div className="min-h-screen h-screen overflow-hidden bg-gradient-to-br from-white via-tiffany-50 to-tiffany-100 flex flex-col">
      ...
      <div className="flex-1 px-4 lg:px-12 py-2 overflow-hidden flex flex-col">
        ...
        {/* Benefits Grid */}
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-4 gap-2 lg:gap-3 max-w-4xl mx-auto">
          {benefits.map((benefit, index) => (
            <div 
              key={index}
              className="bg-white/40 backdrop-blur-md rounded-lg p-2 lg:p-3 border border-white/50 shadow-lg transition-all duration-300 hover:bg-white/70 hover:scale-105 hover:shadow-2xl hover:border-tiffany-300 cursor-default group"
            >
              <h3 className="text-[10px] lg:text-xs font-bold text-tiffany-700 mb-1 group-hover:text-tiffany-800 transition-colors whitespace-nowrap">{benefit.title}</h3>
              <ul className="space-y-0.5">
                {benefit.items.map((item, itemIndex) => (
                  <li key={itemIndex} className="text-[9px] lg:text-[11px] text-tiffany-600 flex items-start gap-1 group-hover:text-tiffany-700 transition-colors whitespace-nowrap">
                    <span className="text-tiffany-500 group-hover:text-tiffany-600">•</span>
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
