...
SELECT 'brands'::public.game_room_type, false
WHERE NOT EXISTS (
  SELECT 1 FROM public.game_rooms WHERE game_type = 'brands'::public.game_room_type
);
...
BEGIN
  ...
  IF room_type IN ('animals', 'rhythms', 'brands') THEN
    max_allowed := 2;
  ELSE
    max_allowed := 1;
  END IF;
