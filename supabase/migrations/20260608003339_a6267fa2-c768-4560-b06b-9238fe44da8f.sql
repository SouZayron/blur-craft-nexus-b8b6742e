...
DECLARE
  ...
  now_local timestamp;
  today date;
  open_at timestamp := '2026-06-05 00:01:00';
  close_at timestamp := '2026-06-16 00:00:00';
  r numeric;
  win_index int;
  prize int := 0;
...
BEGIN
  ...
  -- If boost not allowed (extra spin after a boost), reroll boost into a normal prize
  IF is_boost AND NOT p_allow_boost THEN
    ...
    ELSIF r < 75 THEN win_index := 3; prize := 30;
    ELSIF r < 78 THEN win_index := 4; prize := 40;
    ELSE win_index := 5; prize := 50;
    END IF;
  END IF;

  IF is_boost THEN
    -- BOOST: award a guaranteed random prize (5-50) using the same distribution AND grant an extra spin.
    -- Do NOT update last_spin/streak so the user can spin again immediately.
    r := random() * 79;
    IF r < 40 THEN prize := 5;
    ELSIF r < 60 THEN prize := 10;
    ELSIF r < 70 THEN prize := 20;
    ELSIF r < 75 THEN prize := 30;
    ELSIF r < 78 THEN prize := 40;
    ELSE prize := 50;
    END IF;
    total := prize;
