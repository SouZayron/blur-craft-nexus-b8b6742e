ALTER TABLE public.altavibe_users ADD COLUMN IF NOT EXISTS tz text NOT NULL DEFAULT 'America/Sao_Paulo';
UPDATE public.altavibe_users SET tz = 'Europe/Lisbon' WHERE lower(name) = 'anjinha';
UPDATE public.altavibe_users SET tz = 'Europe/Paris' WHERE lower(name) = 'morgan';
...
BEGIN
  ...
  -- player's own timezone wins; store it if provided and different
  IF tz <> COALESCE(u.tz, 'America/Sao_Paulo') THEN
    BEGIN
      PERFORM now() AT TIME ZONE tz;
      UPDATE public.altavibe_users SET tz = tz WHERE id = u.id;
      u.tz := tz;
    EXCEPTION WHEN OTHERS THEN
      tz := COALESCE(u.tz, 'America/Sao_Paulo');
    END;
  ELSE
    tz := COALESCE(u.tz, 'America/Sao_Paulo');
  END IF;
