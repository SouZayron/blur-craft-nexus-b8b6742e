CREATE OR REPLACE FUNCTION public.altavibe_spin(p_name text, p_tz text DEFAULT 'UTC'::text, p_allow_boost boolean DEFAULT true)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  now_local timestamp;
  today date;
  open_at timestamp := '2026-06-17 00:00:00';
  close_at timestamp := '2026-07-01 00:00:00';
  r numeric;
  win_index int;
  prize int := 0;
...
BEGIN
  ...
  -- ex-winners: can play but win nothing
  is_ex_winner := lower(trim(p_name)) IN ('zaru', 'morgan', 'café com leite');

  r := random() * 79.5;
  IF r < 40 THEN win_index := 0; prize := 50;
  ELSIF r < 60 THEN win_index := 1; prize := 100;
  ELSIF r < 70 THEN win_index := 2; prize := 200;
  ELSIF r < 75 THEN win_index := 3; prize := 300;
  ELSIF r < 78 THEN win_index := 4; prize := 400;
  ELSIF r < 79 THEN win_index := 5; prize := 500;
  ELSE win_index := 6; is_boost := true;
  END IF;

  IF is_boost AND NOT p_allow_boost THEN
    is_boost := false;
    r := random() * 79;
    IF r < 40 THEN win_index := 0; prize := 50;
    ELSIF r < 60 THEN win_index := 1; prize := 100;
  ...
  IF is_boost THEN
    r := random() * 79;
    IF r < 40 THEN prize := 50;
    ELSIF r < 60 THEN prize := 100;
    ELSIF r < 70 THEN prize := 200;
    ELSIF r < 75 THEN prize := 300;
    ELSIF r < 78 THEN prize := 400;
    ELSE prize := 500;
    END IF;
    total := prize;
    ...
    INSERT INTO public.altavibe_logs(user_id, name, prize, bonus, total, is_boost)
    VALUES (u.id, u.name, prize, 0, total, true);

    RETURN jsonb_build_object(
      'win_index', 6, 'prize', prize, 'bonus', 0, 'total', total,
      'is_boost', true,
      'streak', u.streak, 'coins', u.coins, 'last_spin', u.last_spin
    );
  END IF;
